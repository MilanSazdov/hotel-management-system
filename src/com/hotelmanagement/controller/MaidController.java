...
public class MaidController {
    ...
    private int nextMaidId;
    
    private MaidController() {
        maidList = new ArrayList<>();
        loadMaidsFromFile();
        nextMaidId = maidList.size() > 0 ? maidList.get(maidList.size() - 1).getMaidId() + 1 : 1;
    }
        ...
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(path))) {
            for (Maid maid : maidList) {
                bw.write(String.format("%d,%s,%s,%s,%s,%s,%s,%s,%d,%f,%s,%s\n",
                    ...
                    maid.getRoomsId().stream().map(Object::toString).collect(Collectors.joining(";"))
                ));
            }
